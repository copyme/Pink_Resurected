...
int getTiffType(int pi,  /* photometric interpretation */
                ...
                pixtype *pixeltype, imgtype *imagetype) {
              ...
              void **inbuffp)               /* data buffer for this image */
{
  ...
  *pi = photometric;
  ...
  if ((bufIn = (uint8_t *)_TIFFmalloc((tf_bytesperrow + 1) *
                                      sizeof(uint8_t))) == NULL) {
  ...
  LIARdebug("Reading TIFF file %s ; [%d]x[%d], %d bps, %d spp, %d pi, %d sf\n",
            fname, imWidth, imLength, bitspersample, samplesperpixel,
            photometric, sampleformat);
      ...
      case 1:
        for (k = 0; k < samplesperpixel; k++) {
          ...
          for (j = 0; j < imWidth; j++) {
            ...
            cp[j + l * imWidth] =
                (uint8_t)((uint8_t)(bufIn[jj >> 3] & (1 << (7 - jj % 8))) > 0);
          }
        }
        break;
      case 4:
    ...
    for (k = 0; k < samplesperpixel; k++) {
      for (l = 0; l < imLength; l++) {
        ...
        case 1:
          ...
          for (j = 0; j < imWidth; j++) {
            cp[j + l * imWidth] =
                (uint8_t)((uint8_t)(bufIn[j >> 3] & (1 << (7 - j % 8))) > 0);
          }
          break;
        case 4:
          cp = (uint8_t *)(buffp[k]);
  ...
  if ((photometric == PHOTOMETRIC_PALETTE) && (*bps == 8) && (*spp == 1)) {
    uint8_t *pR, *pG, *pB, *q;
    uint8_t **outbuffp;
