...
/* typedef enum */
/* { */
/*     IM_DEFAULT  = -1,     /\* used in arith function when specifying the
 * output type *\/     */
/*     IM_BADPIXEL = 0,      /\* error flag for unknown/illegal pixel types
 * (enum value 0)*\/ */
...
typedef void (*computerankfunc)(PIX_TYPE *in, PIX_TYPE *out, PIX_TYPE *offset,
                                int nx, int nk);
...
long *bresenham3d(int dx, int dy, int dz, int imwidth, int imheight,
                  int imdepth, int *ol, int *period);
long *periodic3d(int dx, int dy, int dz, int imwidth, int imheight, int imdepth,
                 int *ol, int *period);
